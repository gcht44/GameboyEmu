...
{
    bus_write(--(c->sp), value, true);
}

uint8_t pop(s_CPU *c)
{
    return bus_read(c->sp++, true);
}

void push16(s_CPU *c, uint8_t hi, uint8_t lo)
{
    step_timer(1); // for sp = sp - 1; take 1 m-cycles
    push(c, hi);
    push(c, lo);
}
...
void cpu_jr(s_CPU *c)
{
    int8_t value = (int8_t)bus_read(c->pc++, true);
    c->pc += value;
    step_timer(1);
}
...
{
    uint8_t lo = bus_read(c->pc++, true);
    uint8_t hi = bus_read(c->pc++, true);
    push16(c, c->pc >> 8, c->pc & 0xFF);
    c->pc = (hi << 8) | lo;
}
...
void cb_res(uint8_t *reg, uint8_t n)
{
    *reg = *reg & ~(1 << n);
}

void cb_set(uint8_t *reg, uint8_t n)
{
    *reg = *reg | (1 << n);
}
...
uint8_t prefix_cb_exec(s_CPU *c, uint8_t opcode)
{
    ...
    switch (opcode)
    {
        ...
        case 0x05: // RLC L
            ...
            return 8;
        case 0x06: // RLC (HL)
            tmp = bus_read((c->h << 8) | c->l, true);
            cb_rlc(c, &tmp);
            bus_write((c->h << 8) | c->l, tmp, true);
            return 16;
        case 0x07: // RLC A
            cb_rlc(c, &c->a);
            return 8;
        ...
        case 0x45: // BIT 0,L
            ...
            return 8;
        case 0x46: // BIT 0,(HL)
            cb_bit(c, bus_read((c->h << 8) | c->l, true), 0);
            return 16;
        case 0x47: // BIT 0,A
            cb_bit(c, c->a, 0);
            return 8;
        ...
        case 0x4D: // BIT 1,L
            ...
            return 8;
        case 0x4E: // BIT 1,(HL)
            cb_bit(c, bus_read((c->h << 8) | c->l, true), 1);
            return 16;
        case 0x4F: // BIT 1,A
            cb_bit(c, c->a, 1);
            return 8;
        ...
        case 0x55: // BIT 2,L
            ...
            return 8;
        case 0x56: // BIT 2,(HL)
            cb_bit(c, bus_read((c->h << 8) | c->l, true), 2);
            return 16;
        case 0x57: // BIT 2,A
            cb_bit(c, c->a, 2);
            return 8;
        ...
        case 0x5D: // BIT 3,L
            ...
            return 8;
        case 0x5E: // BIT 3,(HL)
            cb_bit(c, bus_read((c->h << 8) | c->l, true), 3);
            return 16;
        case 0x5F: // BIT 3,A
            cb_bit(c, c->a, 3);
            return 8;
        ...
        case 0x65: // BIT 4,L
            ...
            return 8;
        case 0x66: // BIT 4,(HL)
            cb_bit(c, bus_read((c->h << 8) | c->l, true), 4);
            return 16;
        case 0x67: // BIT 4,A
            cb_bit(c, c->a, 4);
            return 8;
        ...
        case 0x6D: // BIT 5,L
            ...
            return 8;
        case 0x6E: // BIT 5,(HL)
            cb_bit(c, bus_read((c->h << 8) | c->l, true), 5);
            return 16;
        case 0x6F: // BIT 5,A
            cb_bit(c, c->a, 5);
            return 8;
        ...
        case 0x75: // BIT 6,L
            ...
            return 8;
        case 0x76: // BIT 6,(HL)
            cb_bit(c, bus_read((c->h << 8) | c->l, true), 6);
            return 16;
        case 0x77: // BIT 6,A
            cb_bit(c, c->a, 6);
            return 8;
        ...
        case 0x7D: // BIT 7,L
            ...
            return 8;
        case 0x7E: // BIT 7,(HL)
            cb_bit(c, bus_read((c->h << 8) | c->l, true), 7);
            return 16;
        case 0x7F: // BIT 7,A
            cb_bit(c, c->a, 7);
            return 8;
...
uint8_t execute(s_CPU *c, uint8_t opcode)
{
    ...
    switch(opcode) {
        case 0x00: // NOP
            return 4;
        case 0x01: // LD BC,nn
            cpu_ld(false, 0, &c->c, bus_read(c->pc++, true));
            cpu_ld(false, 0, &c->b, bus_read(c->pc++, true));
            return 12;
        case 0x02: // LD (BC),A
            addr = ((c->b << 8) | c->c);
        ...
        case 0x07: // RLCA
            ...
            return 4;
        case 0x08: // LD (nn),SP
            lo = bus_read(c->pc++, true);
            hi = bus_read(c->pc++, true);
            addr = (hi << 8) | lo;
            bus_write(addr, c->sp & 0xFF, true);
            bus_write(addr+1, c->sp >> 8, true);
            return 20;
        case 0x09: // ADD HL,BC
            cpu_add16(c, &c->h, &c->l, c->b, c->c);
            return 8;
        case 0x0A: // LD A,(BC)
            cpu_ld(false, 0, &c->a, bus_read((c->b << 8) | c->c, true));
            return 8;
        case 0x0B: // DEC BC
            cpu_dec16(&c->b, &c->c);
            step_timer(1);
        ...
        case 0x10: // STOP
            ...
            return 4;
        case 0x11: // LD DE,nn
            cpu_ld(false, 0, &c->e, bus_read(c->pc++, true));
            cpu_ld(false, 0, &c->d, bus_read(c->pc++, true));
            return 12;
        case 0x12: // LD (DE),A
            addr = ((c->d << 8) | c->e);
            cpu_ld(true, addr, 0, c->a);
        ...
        case 0x19: // ADD HL,DE
            ...
            return 8;
        case 0x1A: // LD A,(DE)
            cpu_ld(false, 0, &c->a, bus_read((c->d << 8) | c->e, true));
            return 8;
        case 0x1B: // DEC DE
            cpu_dec16(&c->d, &c->e);
            step_timer(1);
        ...
        case 0x20: // JR NZ,n
            ...
            return 8;
        case 0x21: // LD HL,nn
            cpu_ld(false, 0, &c->l, bus_read(c->pc++, true));
            cpu_ld(false, 0, &c->h, bus_read(c->pc++, true));
            return 12;
        case 0x22: // LD (HL+),A
            addr = ((c->h << 8) | c->l);
            cpu_ld(true, addr, 0, c->a);
        ...
        case 0x29: // ADD HL,HL
            ...
            return 8;
        case 0x2A: // LD A,(HL+)
            cpu_ld(false, 0, &c->a, bus_read((c->h << 8) | c->l, true));
            cpu_inc16(&c->h, &c->l);
            return 8;
        case 0x2B: // DEC HL
            cpu_dec16(&c->h, &c->l);
        ...
        case 0x33: // INC SP
            ...
            return 8;
        case 0x34: // INC (HL)
            hi = bus_read((c->h << 8) | c->l, true);
            bus_write((c->h << 8) | c->l, cpu_inc(c, hi), true);
            return 12;
        case 0x35: // DEC (HL)
            hi = bus_read((c->h << 8) | c->l, true);
            bus_write((c->h << 8) | c->l, cpu_dec(c, hi), true);
            return 12;
        case 0x36: // LD (HL),n
            cpu_ld(true, (c->h << 8) | c->l, 0, bus_read(c->pc++, true));
            return 8;
        case 0x37: // SCF
            cpu_scf(c);
            return 4;
        ...
        case 0x3D: // DEC A
            ...
            return 4;
        case 0x3E: // LD A,n
            cpu_ld(false, 0, &c->a, bus_read(c->pc++, true));
            return 8;
        case 0x3F: // CCF
            cpu_ccf(c);
            return 4;
        ...
        case 0x45: // LD B,L
            ...
            return 4;
        case 0x46: // LD B,(HL)
            cpu_ld(false, 0, &c->b, bus_read((c->h << 8) | c->l, true));
            return 8;
        case 0x47: // LD B,A
        ...
        case 0x4D: // LD C,L
            ...
            return 4;
        case 0x4E: // LD C,(HL)
            cpu_ld(false, 0, &c->c, bus_read((c->h << 8) | c->l, true));
            return 8;
        case 0x4F: // LD C,A
        ...
        case 0x55: // LD D,L
            ...
            return 4;
        case 0x56: // LD D,(HL)
            cpu_ld(false, 0, &c->d, bus_read((c->h << 8) | c->l, true));
            return 8;
        case 0x57: // LD D,A
        ...
        case 0x5D: // LD E,L
            ...
            return 4;
        case 0x5E: // LD E,(HL)
            cpu_ld(false, 0, &c->e, bus_read((c->h << 8) | c->l, true));
            return 8;
        case 0x5F: // LD E,A
        ...
        case 0x65: // LD H,L
            ...
            return 4;
        case 0x66: // LD H,(HL)
            cpu_ld(false, 0, &c->h, bus_read((c->h << 8) | c->l, true));
            return 8;
        case 0x67: // LD H,A
        ...
        case 0x6D: // LD L,L
            ...
            return 4;
        case 0x6E: // LD L,(HL)
            cpu_ld(false, 0, &c->l, bus_read((c->h << 8) | c->l, true));
            return 8;
        case 0x6F: // LD L,A
        ...
        case 0x85: // ADD A,L
            ...
            return 4;
        case 0x86: // ADD A,(HL)
            cpu_add(c, bus_read((c->h << 8) | c->l, true));
            return 8;
        case 0x87: // ADD A,A
            cpu_add(c, c->a);
            return 4;
        ...
        case 0x8D: // ADC A,L
            ...
            return 4;
        case 0x8E: // ADC A,(HL)
            cpu_adc(c, bus_read((c->h << 8) | c->l, true));
            return 8;
        case 0x8F: // ADC A,A
            cpu_adc(c, c->a);
            return 4;
        ...
        case 0x95: // SUB L
            ...
            return 4;
        case 0x96: // SUB (HL)
            cpu_sub(c, bus_read((c->h << 8) | c->l, true));
            return 8;
        case 0x97: // SUB A
            cpu_sub(c, c->a);
            return 4;
        ...
        case 0x9D: // SBC A,L
            ...
            return 4;
        case 0x9E: // SBC A,(HL)
            cpu_sbc(c, bus_read((c->h << 8) | c->l, true));
            return 8;
        case 0x9F: // SBC A,A
            cpu_sbc(c, c->a);
            return 4;
        ...
        case 0xA5: // AND L
            ...
            return 4;
        case 0xA6: // AND (HL)
            cpu_and(c, bus_read((c->h << 8) | c->l, true));
            return 8;
        case 0xA7: // AND A
            cpu_and(c, c->a);
            return 4;
        ...
        case 0xAD: // XOR L
            ...
            return 4;
        case 0xAE: // XOR (HL)
            cpu_xor(c, bus_read((c->h << 8) | c->l, true));
            return 8;
        case 0xAF: // XOR A
            cpu_xor(c, c->a);
            return 4;
        ...
        case 0xB5: // OR L
            ...
            return 4;
        case 0xB6: // OR (HL)
            cpu_or(c, bus_read((c->h << 8) | c->l, true));
            return 8;
        case 0xB7: // OR A
            cpu_or(c, c->a);
            return 4;
        ...
        case 0xBD: // CP L
            ...
            return 4;
        case 0xBE: // CP (HL)
            cpu_cp(c, bus_read((c->h << 8) | c->l, true));
            return 8;
        case 0xBF: // CP A
            cpu_cp(c, c->a);
            return 4;
        ...
        case 0xCA: // JP Z,nn
            ...
            return 12;
        case 0xCB: // CB prefix
            return prefix_cb_exec(c, bus_read(c->pc++, true));
        case 0xCC: // CALL Z,nn
            if (c->zf)
            {
                cpu_call(c);
        ...
        case 0xF0: // LDH A,(n)
            lo = bus_read(c->pc++, true);
            cpu_ld(false, 0, &c->a, bus_read(0xFF00 | lo, true));
            return 12;
        case 0xF1: // POP AF
            lo = pop(c);
            c->zf = (lo & 0x80) >> 7;
            ...
            return 12;
        case 0xF2: // LD A,(C)
            cpu_ld(false, 0, &c->a, bus_read(0xFF00 + c->c, true));
            return 8;
        case 0xF3: // DI
            c->ime = 0;
            c->ime_pending = 0;
        ...
        case 0xF5: // PUSH AF
            ...
            return 16;
        case 0xF6: // OR n
            cpu_or(c, bus_read(c->pc++, true));
            return 8;
        case 0xF7: // RST 30H
            cpu_rst(c, 0x0030);
            return 16;
        case 0xF8: // LD HL,SP+n
            lo = bus_read(c->pc++, true); // M2
            addr = (int16_t)c->sp + (int8_t)lo; // M3
            c->zf = 0;
            c->nf = 0;
            c->hf = ((((int16_t)c->sp & 0xF) + ((int16_t)lo & 0xF)) > 0xF);
            c->cy = ((((int16_t)c->sp & 0xFF) + ((int16_t)lo & 0xFF)) > 0xFF);
        ...
        case 0xFA: // LD A,(nn)
            lo = bus_read(c->pc++, true);
            hi = bus_read(c->pc++, true);
            cpu_ld(false, 0, &c->a, bus_read((hi << 8) | lo, true));
            return 16;
        case 0xFB: // EI
            c->ime_pending = 1;
            return 4;
...
{
    bus_write(0xFF0F, bus_read(0xFF0F, false) | it, false);
}

void irq(s_CPU *c, uint8_t interrupt_flag)
{
    uint16_t vector = 0;
    uint8_t interrupt_exec = interrupt_flag & ie;
    uint8_t bit;

    if (interrupt_exec & 0x01) // VBLANK
    {
        vector = 0x0040;
        bit = 0;
    }
    else if (interrupt_exec & 0x02) // LCD
    {
        vector = 0x0048;
        bit = 1;
    }
    else if (interrupt_exec & 0x04) // TIMER
    {
        vector = 0x0050;
        bit = 2;
    }
    else if (interrupt_exec & 0x08) // SERIAL
    {
        vector = 0x0058;
        bit = 3;
    }
    else if (interrupt_exec & 0x10) // JOYPAD
    {
        vector = 0x0060;
        bit = 4;
    }
    if (vector > 0)
    {
        c->ime = 0;
        c->halt = 0;
        bus_write(0xFF0F, interrupt_flag & ~(1 << bit), true);
        // Call vector
        push16(c, c->pc >> 8, c->pc & 0xFF);
        c->pc = vector;
        cycles += 20;
...
void step_emu(s_CPU *c)
{
    ...
    {
        printf("%c", bus_read(0xFF01, false));
        bus_write(0xFF02, 0, false);
    }
    ...
    {
        uint8_t opcode = bus_read(c->pc++, true);
        cycles += execute(c, opcode);
    }
    else if (c->halt)
        step_timer(1);
}